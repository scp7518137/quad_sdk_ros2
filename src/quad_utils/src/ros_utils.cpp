...
void interpHeader(std_msgs::msg::Header header_1, std_msgs::msg::Header header_2,
  ...
  // Compute the correct rclcpp::Time corresponding to t_interp
  t_interp = std::max(std::min(t_interp, 1.0), 0.0);
  rclcpp::Duration state_duration = rclcpp::Time(header_2.stamp) - rclcpp::Time(header_1.stamp);
  rclcpp::Duration interp_duration = rclcpp::Duration::from_seconds(t_interp * state_duration.seconds());
  interp_header.stamp = rclcpp::Time(header_1.stamp) + interp_duration;
}
...
void interpRobotPlan(quad_msgs::msg::RobotPlan msg, double t,
  ...
  rclcpp::Time t_ros = t0_ros + rclcpp::Duration::from_seconds(t);
  ...
  if (t >= 0) {
    for (int i = 0; i < msg.states.size() - 1; i++) {
      index = i;
      if (rclcpp::Time(msg.states[i].header.stamp) <= t_ros &&
          t_ros < rclcpp::Time(msg.states[i + 1].header.stamp)) {
        break;
      }
    }
  }
  ...
  double t1, t2;
  rclcpp::Duration t1_ros = rclcpp::Time(state_1.header.stamp) - t0_ros;
  t1 = t1_ros.seconds();
  rclcpp::Duration t2_ros = rclcpp::Time(state_2.header.stamp) - t0_ros;
  t2 = t2_ros.seconds();
...
//   rclcpp::Duration traj_duration = tf_ros - t0_ros;

//   t = std::max(std::min(t, traj_duration.seconds()), 0.0);
//   rclcpp::Time t_ros = t0_ros + rclcpp::Duration::from_seconds(t);
...
//   // Compute t_interp = [0,1]
//   double t1, t2;
//   rclcpp::Duration t1_ros = state_1.header.stamp - t0_ros;
//   t1 = t1_ros.seconds();
//   rclcpp::Duration t2_ros = state_2.header.stamp - t0_ros;
//   t2 = t2_ros.seconds();
//   double t_interp = (t - t1) / (t2 - t1);
