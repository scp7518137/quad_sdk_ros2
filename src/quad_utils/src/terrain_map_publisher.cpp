...
TerrainMapPublisher::TerrainMapPublisher(rclcpp::Node::SharedPtr node)
    : terrain_map_(grid_map::GridMap(
  ...
  quad_utils::loadROSParamDefault(node_, "topics/terrain_map_raw", terrain_map_topic,
                        std::string("/terrain_map_raw"));
  quad_utils::loadROSParamDefault(node_, "/map_frame", map_frame_, std::string("map"));
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/update_rate", update_rate_, 10.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/obstacle_x", obstacle_.x, 2.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/obstacle_y", obstacle_.y, 0.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/obstacle_height", obstacle_.height,
                   0.5);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/obstacle_radius", obstacle_.radius,
                   1.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step1_x", step1_.x, 4.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step1_height", step1_.height, 0.3);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step2_x", step2_.x, 4.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step2_height", step2_.height, 0.3);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/resolution", resolution_, 0.2);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/update_rate", update_rate_, 10.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/map_data_source",
                        map_data_source_, std::string("internal"));
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/terrain_type", terrain_type_,
                        std::string("slope"));
  // Setup pubs and subs
  terrain_map_pub_ =
      node_->create_publisher<grid_map_msgs::msg::GridMap>(terrain_map_topic, 1);

  // Add image subscriber if data source requests an image
  if (map_data_source_.compare("image") == 0) {
    quad_utils::loadROSParamDefault(node_, "topics/image", image_topic,
                           std::string("/image_publisher/image"));
    quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/min_height", min_height_, 0.0);
    quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/max_height", max_height_, 1.0);
    image_sub_ = node_->create_subscription<sensor_msgs::msg::Image>(image_topic, 1,
                               std::bind(&TerrainMapPublisher::loadMapFromImage, this, std::placeholders::_1));
  }

  // Initialize the elevation layer on the terrain map
  terrain_map_.setBasicLayers(
      {"z", "nx", "ny", "nz", "z_filt", "nx_filt", "ny_filt", "nz_filt"});
}
...
void TerrainMapPublisher::updateParams() {
  ...
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step1_x", step1_.x, 4.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step1_height", step1_.height, 0.3);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step2_x", step2_.x, 6.0);
  quad_utils::loadROSParamDefault(node_, "terrain_map_publisher/step2_height", step2_.height, -0.3);
}
...
void TerrainMapPublisher::createMap() {
  ...
  RCLCPP_INFO(node_->get_logger(), "Created map with size %f x %f m (%i x %i cells).",
...
void TerrainMapPublisher::loadMapFromCSV() {
  ...
  for (grid_map::GridMapIterator iterator(terrain_map_); !iterator.isPastEnd();
    ...
    terrain_map_.at("z", *iterator) =
        z_data[(y_size - 1) - index[1]][(x_size - 1) - index[0]];
    terrain_map_.at("nx", *iterator) =
        nx_data[(y_size - 1) - index[1]][(x_size - 1) - index[0]];
    terrain_map_.at("ny", *iterator) =
        ny_data[(y_size - 1) - index[1]][(x_size - 1) - index[0]];
    terrain_map_.at("nz", *iterator) =
        nz_data[(y_size - 1) - index[1]][(x_size - 1) - index[0]];

    terrain_map_.at("z_filt", *iterator) =
        z_data_filt[(y_size - 1) - index[1]][(x_size - 1) - index[0]];
    terrain_map_.at("nx_filt", *iterator) =
        nx_data_filt[(y_size - 1) - index[1]][(x_size - 1) - index[0]];
    terrain_map_.at("ny_filt", *iterator) =
        ny_data_filt[(y_size - 1) - index[1]][(x_size - 1) - index[0]];
    terrain_map_.at("nz_filt", *iterator) =
        nz_data_filt[(y_size - 1) - index[1]][(x_size - 1) - index[0]];
  }
}
...
void TerrainMapPublisher::loadMapFromImage(const sensor_msgs::msg::Image::SharedPtr msg) {
  ...
  if (!map_initialized_) {
    grid_map::GridMapRosConverter::initializeFromImage(*msg, resolution_,
                                                       terrain_map_);
    RCLCPP_INFO(node_->get_logger(), "Initialized map with size %f x %f m (%i x %i cells).",
             terrain_map_.getLength().x(), terrain_map_.getLength().y(),
             terrain_map_.getSize()(0), terrain_map_.getSize()(1));
  ...
  // Add the data layers
  grid_map::GridMapRosConverter::addLayerFromImage(*msg, "z", terrain_map_,
...
void TerrainMapPublisher::spin() {
  ...
  if (map_data_source_.compare("image") == 0) {
    ...
    while (rclcpp::ok()) {
      if (rclcpp::wait_for_message<sensor_msgs::msg::Image>(
              image_msg, node_, "/image_publisher/image")) {
        break;
      }
