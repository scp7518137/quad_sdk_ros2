...
QuadKD::QuadKD(rclcpp::Node::SharedPtr node, std::string ns) : node_(node) { initModel("/" + ns + "/"); }
...
bool QuadKD::legbaseToFootIKLegbaseFrame(int leg_index,
  ...
  if (abs(temp2) > acos_eps) {
    ...
    RCLCPP_DEBUG_THROTTLE(
      ...
      *std::make_shared<rclcpp::Clock>(),
      500,
      "Foot location too far for hip, choosing closest" " alternative \n");
    is_exact = false;
  ...
  if (temp3 > acos_eps || temp3 < -acos_eps) {
    ...
    RCLCPP_DEBUG_THROTTLE(
      ...
      *std::make_shared<rclcpp::Clock>(),
      500,
      "Foot location too far for knee, choosing closest" " alternative \n");
    is_exact = false;
  ...
  RCLCPP_DEBUG_THROTTLE(
    ...
    *std::make_shared<rclcpp::Clock>(),
    500,
    "Knee limit exceeded, clamping to %5.3f \n", q2);
  }

  // q1 is undefined if q2=0, resolve this
  if (q2 == 0) {
    q1 = 0;
    // ROS_DEBUG_THROTTLE(0.5, "Hip value undefined (in singularity), setting to" " %5.3f \n", q1);
    RCLCPP_DEBUG_THROTTLE(
      node_->get_logger(),
      *std::make_shared<rclcpp::Clock>(),
      500,
      "Hip value undefined (in singularity), setting to" " %5.3f \n", q1);
    is_exact = false;
  }
