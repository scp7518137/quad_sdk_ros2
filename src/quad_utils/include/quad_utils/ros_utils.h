 ...
 * @brief Gets the relative age of a timestamped header
 * @param[in] header ROS Header that we wish to compute the age of
 * @return Age in ms (compared to rclcpp::Time::now())
 */
inline double getROSMessageAgeInMs(std_msgs::msg::Header header) {
 ...
 * @return Time in plan (compared to rclcpp::Time::now())
 */
inline double getDurationSinceTime(rclcpp::Time plan_start) {
  return (rclcpp::Clock(RCL_ROS_TIME).now() - plan_start).seconds();
}

/**
 * @brief Gets the index associated with a given time
 * @param[out] index Index in plan (compared to rclcpp::Time::now())
 * @param[out] first_element_duration Time duration to next index in plan
 * (compared to rclcpp::Time::now())
 * @param[in] plan_start ROS Time to to compare to
...
/**
 * @brief Normalize a ROS1-style parameter name ("ns/name" or "/ns/name") into
 * the ROS2 convention ("ns.name") so that params loaded from nested YAML
 * (under ros__parameters) can be resolved.
 * @param[in] paramName ROS1-style parameter name
 * @return Normalized ROS2 parameter name
 */
inline std::string normalizeParamName(std::string paramName) {
  if (!paramName.empty() && paramName.front() == '/') {
    paramName.erase(0, 1);
  }
...
inline bool loadROSParam(rclcpp::Node::SharedPtr nh, std::string paramName,
                         ParamType &varName) {
  paramName = normalizeParamName(paramName);
  rclcpp::Parameter param;
  if (!nh->get_parameter(paramName, param)) {
    RCLCPP_ERROR(nh->get_logger(), "Can't find param %s from parameter server", paramName.c_str());
    return false;
  }
  // ROS2 参数严格按类型：整数 override（如 update_rate: 100）会被声明为
  // PARAMETER_INTEGER，直接按 double 读取会抛 InvalidParameterTypeException。
  // 这里还原 ROS1 的 int<->double 宽松强转语义。
  if constexpr (std::is_same_v<ParamType, double> ||
                std::is_same_v<ParamType, float>) {
    varName = (param.get_type() == rclcpp::PARAMETER_INTEGER)
                  ? static_cast<ParamType>(param.as_int())
                  : static_cast<ParamType>(param.as_double());
  } else if constexpr (std::is_same_v<ParamType, int> ||
                       std::is_same_v<ParamType, int64_t>) {
    varName = (param.get_type() == rclcpp::PARAMETER_DOUBLE)
                  ? static_cast<ParamType>(param.as_double())
                  : static_cast<ParamType>(param.as_int());
  } else {
    // 其余类型（string / bool / 各类 vector）要求声明类型严格匹配
    if (!nh->get_parameter(paramName, varName)) {
      RCLCPP_ERROR(nh->get_logger(), "Can't find param %s from parameter server", paramName.c_str());
      return false;
    }
  }
 ...
 */
template <class ParamType>
inline bool loadROSParamDefault(rclcpp::Node::SharedPtr nh, std::string paramName,
                                ParamType &varName, ParamType defaultVal) {
  paramName = normalizeParamName(paramName);
  rclcpp::Parameter param;
  if (!nh->get_parameter(paramName, param)) {
    varName = defaultVal;
    RCLCPP_INFO(nh->get_logger(), "Can't find param %s on rosparam server, loading default value.",
                            paramName.c_str());
    return false;
  }
  // 与 loadROSParam 相同的 int<->double 宽容转换
  if constexpr (std::is_same_v<ParamType, double> ||
                std::is_same_v<ParamType, float>) {
    varName = (param.get_type() == rclcpp::PARAMETER_INTEGER)
                  ? static_cast<ParamType>(param.as_int())
...
/**
 * @brief Convert eigen vector to geometry_msgs::msg::Vector3
...
/**
 * @brief Convert geometry_msgs::msg::Vector3 vector to eigen vector
...
/**
 * @brief Convert eigen vector to geometry_msgs::msg::Point
 * @param[in] vec Eigen vector
...
/**
 * @brief Convert geometry_msgs::msg::Point vector to eigen vector
