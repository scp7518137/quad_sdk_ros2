...
class GlobalBodyPlan {
 public:
   ...
   */
  inline void setComputedTimestamp(rclcpp::Time timestamp) {
   ...
   */
  inline double setPublishedTimestamp(rclcpp::Time timestamp) {
   ...
   */
  inline rclcpp::Time getPublishedTimestamp() const {
 ...
 private:
  ...
  rclcpp::Time computed_timestamp_;

  /// Time stamp for when plan was published (not unique)
  rclcpp::Time published_timestamp_;
