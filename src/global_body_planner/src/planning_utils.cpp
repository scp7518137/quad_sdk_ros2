...
void eigenToFullState(const Eigen::VectorXd &s_eig, FullState &s) {
  if (s_eig.size() != 12) {
    RCLCPP_ERROR(rclcpp::get_logger("planning_utils"),
                 "Eigen::VectorXd is incorrect size");
  }
...
void vectorToFullState(const std::vector<double> &v, FullState &s) {
  if (v.size() != 12) {
    RCLCPP_ERROR(rclcpp::get_logger("planning_utils"),
                 "std::vector<double> is incorrect size");
  }
  s.pos[0] = v[0];
  s.pos[1] = v[1];
  s.pos[2] = v[2];
...
bool refineStance(const State &s, int phase, Action &a,
  ...
  while (!(grf_valid && friction_cone_valid && final_state_valid &&
    ...
    // MIT(jcnorby): sample surface normal from ground projection
    // (non-filtered)
    grf_valid =
...
bool isValidState(const State &s, const PlannerConfig &planner_config,
  ...
  // MIT(jcnorby): Add getRollFromState
  ...
  // MIT(jcnorby): allow alternative gaits, check region around foot location
...
void publishStateActionPair(const State &s, const Action &a,
  ...
  state_action_line.header.stamp = rclcpp::Clock(RCL_ROS_TIME).now();
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (interp_primitive_id[i] == FLIGHT) {
      ...
      static rclcpp::Clock throttle_clock(RCL_ROS_TIME);
      RCLCPP_WARN_THROTTLE(rclcpp::get_logger("planning_utils"),
                          throttle_clock, 1,
                          "Invalid primitive ID received in planning utils");
    }
