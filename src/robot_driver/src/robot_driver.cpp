...
RobotDriver::RobotDriver(rclcpp::Node::SharedPtr node, int argc, char **argv) {
  nh_ = node;
  argc_ = argc;
  argv_ = argv;
  ...
  // Setup pubs and subs
  local_plan_sub_ = nh_->create_subscription<quad_msgs::msg::RobotPlan>(
      local_plan_topic, rclcpp::QoS(1),
      std::bind(&RobotDriver::localPlanCallback, this,
                std::placeholders::_1));
  control_mode_sub_ = nh_->create_subscription<std_msgs::msg::UInt8>(
      control_mode_topic, rclcpp::QoS(1),
      std::bind(&RobotDriver::controlModeCallback, this,
                std::placeholders::_1));
  single_joint_cmd_sub_ = nh_->create_subscription<geometry_msgs::msg::Vector3>(
      single_joint_cmd_topic, rclcpp::QoS(1),
      std::bind(&RobotDriver::singleJointCommandCallback, this,
                std::placeholders::_1));
  remote_heartbeat_sub_ = nh_->create_subscription<std_msgs::msg::Header>(
      remote_heartbeat_topic, rclcpp::QoS(1),
      std::bind(&RobotDriver::remoteHeartbeatCallback, this,
                std::placeholders::_1));
  control_restart_flag_sub_ = nh_->create_subscription<std_msgs::msg::Bool>(
      control_restart_flag_topic, rclcpp::QoS(1),
      std::bind(&RobotDriver::controlRestartFlagCallback, this,
                std::placeholders::_1));
  grf_pub_ = nh_->create_publisher<quad_msgs::msg::GRFArray>(
      grf_topic, rclcpp::QoS(1));
  leg_command_array_pub_ = nh_->create_publisher<quad_msgs::msg::LegCommandArray>(
      leg_command_array_topic, rclcpp::QoS(1));
  robot_heartbeat_pub_ = nh_->create_publisher<std_msgs::msg::Header>(
      robot_heartbeat_topic, rclcpp::QoS(1));
  trajectry_robot_state_pub_ = nh_->create_publisher<quad_msgs::msg::RobotState>(
      trajectory_state_topic, rclcpp::QoS(1));

  // Set up pubs and subs dependent on robot layer
  if (is_hardware_) {
    RCLCPP_INFO(nh_->get_logger(), "Loading hardware robot driver");
    mocap_sub_ = nh_->create_subscription<geometry_msgs::msg::PoseStamped>(
        mocap_topic, rclcpp::QoS(1),
        std::bind(&RobotDriver::mocapCallback, this,
                  std::placeholders::_1));
    robot_state_pub_ = nh_->create_publisher<quad_msgs::msg::RobotState>(
        robot_state_topic, rclcpp::QoS(1));
    imu_pub_ = nh_->create_publisher<sensor_msgs::msg::Imu>(imu_topic,
                                                            rclcpp::QoS(1));
    joint_state_pub_ = nh_->create_publisher<sensor_msgs::msg::JointState>(
        joint_state_topic, rclcpp::QoS(1));
  } else {
    RCLCPP_INFO(nh_->get_logger(), "Loading sim robot driver");
    robot_state_sub_ = nh_->create_subscription<quad_msgs::msg::RobotState>(
        robot_state_topic, rclcpp::QoS(1),
  ...
  // Initialize timing
  last_robot_state_msg_.header.stamp = nh_->now();
  t_pub_ = nh_->now();
...
void RobotDriver::initStateEstimator() {
  if (estimator_id_ == "comp_filter") {
    ...
    RCLCPP_ERROR_STREAM(nh_->get_logger(),
                        "Invalid estimator id " << estimator_id_
                                                << ", returning nullptr");
...
void RobotDriver::initLegController() {
  if (controller_id_ == "inverse_dynamics") {
    leg_controller_ = std::make_shared<InverseDynamicsController>(nh_);
  } else if (controller_id_ == "grf_pid") {
    leg_controller_ = std::make_shared<GrfPidController>(nh_);
  } else if (controller_id_ == "joint") {
    leg_controller_ = std::make_shared<JointController>(nh_);
  } else {
    RCLCPP_ERROR_STREAM(nh_->get_logger(),
                        "Invalid controller id " << controller_id_
                                                << ", returning nullptr");
...
void RobotDriver::controlModeCallback(const std_msgs::msg::UInt8::ConstSharedPtr &msg) {
...
void RobotDriver::singleJointCommandCallback(
    const geometry_msgs::msg::Vector3::ConstSharedPtr &msg) {
...
void RobotDriver::localPlanCallback(const quad_msgs::msg::RobotPlan::ConstSharedPtr &msg) {
  last_local_plan_msg_ = msg;

  rclcpp::Time t_now = nh_->now();
  double round_trip_time_diff =
      (t_now - rclcpp::Time(last_local_plan_msg_->state_timestamp)).seconds();
...
void RobotDriver::mocapCallback(
  ...
  double t_diff_mocap_msg =
      (rclcpp::Time(msg->header.stamp) -
       rclcpp::Time(last_mocap_msg_->header.stamp)).seconds();
  double t_mocap_ros_latency = (t_now - rclcpp::Time(msg->header.stamp)).seconds();
  ...
  if (abs(t_diff_mocap_msg - 1.0 / mocap_rate_) < mocap_dropout_threshold_) {
    ...
    RCLCPP_WARN_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 100,
...
void RobotDriver::robotStateCallback(
    const quad_msgs::msg::RobotState::ConstSharedPtr &msg) {
  last_robot_state_msg_ = *msg;
}

void RobotDriver::remoteHeartbeatCallback(
    const std_msgs::msg::Header::ConstSharedPtr &msg) {
  // Get the current time and compare to the message time
  double remote_heartbeat_sent_time = rclcpp::Time(msg->stamp).seconds();
  remote_heartbeat_received_time_ = nh_->now().seconds();
  double t_latency =
      remote_heartbeat_received_time_ - remote_heartbeat_sent_time;

  // RCLCPP_INFO_THROTTLE(nh_->get_logger(), 1.0,
  //                      "Remote latency (+ clock skew) = %6.4fs", t_latency);
}
...
void RobotDriver::checkMessagesForSafety() {
  ...
  if (abs(nh_->now().seconds() - remote_heartbeat_received_time_) >=
          heartbeat_timeout_ &&
      remote_heartbeat_received_time_ != std::numeric_limits<double>::max()) {
  ...
  if (!is_hardware_ &&
      abs(nh_->now().seconds() - last_state_time_) >= state_timeout_ &&
      last_state_time_ != std::numeric_limits<double>::max()) {
    control_mode_ = SAFETY;
    transition_timestamp_ = nh_->now();
...
bool RobotDriver::updateControl() {
  ...
  if (control_mode_ == SAFETY) {
    ...
    rclcpp::Duration duration = nh_->now() - transition_timestamp_;
    ...
    RCLCPP_WARN_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 500,
  ...
  for (int i = 0; i < num_feet_; ++i) {
    for (int j = 0; j < 3; ++j) {
      ...
      if (abs(effort) >= torque_limits_[j]) {
        RCLCPP_WARN(nh_->get_logger(),
                    "Leg %d motor %d: total effort = %5.3f Nm exceeds "
                    "threshold of %5.3f Nm",
...
void RobotDriver::publishControl(bool is_valid) {
  ...
  // if ((nh_->now() - leg_command_array_msg_.header.stamp).seconds()
  // >= 1.0/publish_rate_) {
  leg_command_array_msg_.header.stamp = nh_->now();
  ...
  if (is_hardware_ && is_valid) {
    rclcpp::Time t_start = nh_->now();
    hardware_interface_->send(leg_command_array_msg_, user_tx_data_);
    rclcpp::Time t_end = nh_->now();

    RCLCPP_INFO_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "t_diff_mb_send = %6.4f",
...
void RobotDriver::publishHeartbeat() {
  // Publish hearbeat
  if ((nh_->now() - rclcpp::Time(last_robot_heartbeat_msg_.stamp)).seconds() >=
      1.0 / publish_rate_) {
    last_robot_heartbeat_msg_.stamp = nh_->now();
