...
InverseDynamicsController::InverseDynamicsController(rclcpp::Node::SharedPtr node)
    : LegController(node) {}
...
bool InverseDynamicsController::computeLegCommandArray(
  ...
  if ((last_local_plan_msg_ == nullptr) ||
      ((rclcpp::Clock(RCL_ROS_TIME).now() -
        rclcpp::Time(last_local_plan_msg_->header.stamp)).seconds() >=
       0.1)) {
    return false;
  } else {
    ...
    double t_now = (rclcpp::Clock(RCL_ROS_TIME).now() -
                    rclcpp::Time(last_local_plan_msg_->state_timestamp))
                       .seconds();  // Use time of state - RECOMMENDED
    // double t_now = (rclcpp::Clock(RCL_ROS_TIME).now() - last_local_plan_time_)
    //                     .seconds();  // Use time of plan receipt
    // double t_now = (rclcpp::Clock(RCL_ROS_TIME).now() - t_first_state)
    //                     .seconds();  // Use time of first state in plan
    ...
    if ((t_now <
        ...
        (t_now >
         (rclcpp::Time(last_local_plan_msg_->states.back().header.stamp) -
          t_first_state)
             .seconds())) {
      RCLCPP_ERROR(rclcpp::get_logger("robot_driver"),
