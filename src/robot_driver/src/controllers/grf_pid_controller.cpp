...
GrfPidController::GrfPidController(rclcpp::Node::SharedPtr node)
  ...
  t_old_ = rclcpp::Clock(RCL_ROS_TIME).now();
  ang_des_.setZero();
}
...
bool GrfPidController::computeLegCommandArray(
  ...
  rclcpp::Time t_now = rclcpp::Clock(RCL_ROS_TIME).now();
  pos_error_int_ += pos_error * std::min((t_now - t_old_).seconds(), 0.01);
  ang_error_int_ += ang_error * std::min((t_now - t_old_).seconds(), 0.01);
