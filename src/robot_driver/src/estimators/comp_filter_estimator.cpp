...
void CompFilterEstimator::init(rclcpp::Node::SharedPtr nh) {
  nh_ = nh;
...
bool CompFilterEstimator::updateOnce(
  ...
  rclcpp::Time state_timestamp = rclcpp::Clock(RCL_ROS_TIME).now();
  ...
  if (last_mocap_msg_ == nullptr) {
    RCLCPP_WARN_THROTTLE(rclcpp::get_logger("robot_driver"), *nh_->get_clock(), 1000,
