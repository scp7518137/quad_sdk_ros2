...
TEST(LocalPlannerTest, noInputCase) {
  auto nh = makeTestNode("local_planner_tester");
...
TEST(LocalFootStepPlanner, baseCase) {
  auto nh = makeTestNode("local_footstep_planner_tester");
  int N;
  nh->get_parameter_or("/local_planner/horizon_length", N, 24);
  ...
  nh->get_parameter_or("/local_footstep_planner/duty_cycles", duty_cycles,
                       std::vector<double>{0.5, 0.5, 0.5, 0.5});
  std::vector<double> phase_offsets;
  nh->get_parameter_or("/local_footstep_planner/phase_offsets", phase_offsets,
  ...
  std::shared_ptr<quad_utils::QuadKD> quadKD =
      std::make_shared<quad_utils::QuadKD>(nh);
