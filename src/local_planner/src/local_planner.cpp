...
LocalPlanner::LocalPlanner(rclcpp::Node::SharedPtr nh)
  ...
  // Setup pubs and subs
  terrain_map_sub_ = nh_->create_subscription<grid_map_msgs::msg::GridMap>(
      terrain_map_topic, 1,
      std::bind(&LocalPlanner::terrainMapCallback, this,
                std::placeholders::_1));
  body_plan_sub_ = nh_->create_subscription<quad_msgs::msg::RobotPlan>(
      body_plan_topic, 1,
      std::bind(&LocalPlanner::robotPlanCallback, this,
                std::placeholders::_1));
  ...
  local_plan_pub_ =
      nh_->create_publisher<quad_msgs::msg::RobotPlan>(local_plan_topic, 1);
  foot_plan_discrete_pub_ =
      nh_->create_publisher<quad_msgs::msg::MultiFootPlanDiscrete>(
          foot_plan_discrete_topic, 1);
  foot_plan_continuous_pub_ =
      nh_->create_publisher<quad_msgs::msg::MultiFootPlanContinuous>(
  ...
  // Load system parameters from launch file (not in config file)
  nh_->get_parameter_or("local_planner/use_twist_input", use_twist_input_,
                        false);
  ...
  cmd_vel_.resize(6);
  cmd_vel_.setZero();
  initial_timestamp_ = nh_->now();
  last_cmd_vel_msg_time_ = rclcpp::Time(0, 0, nh_->get_clock()->get_clock_type());
  first_plan_ = true;
...
void LocalPlanner::initLocalBodyPlanner() {
  ...
  if (robot_name_ == "spirit") {
    ...
    RCLCPP_WARN(nh_->get_logger(), "WRONG ROBOT TYPE");
  }
...
void LocalPlanner::initLocalFootstepPlanner() {
  ...
  if (grf_weight > 1 || grf_weight < 0) {
    ...
    RCLCPP_WARN(nh_->get_logger(), "Invalid grf weight, clamping to %4.2f",
                grf_weight);
  }
...
void LocalPlanner::cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
  ...
  last_cmd_vel_msg_time_ = nh_->now();
}
...
void LocalPlanner::getReference() {
  if (first_plan_) {
    ...
    if (use_twist_input_) {
      initial_timestamp_ = nh_->now() - rclcpp::Duration::from_seconds(1e-6);
    }
  }
  ...
  if (use_twist_input_) {
    ...
    rclcpp::Duration time_elapsed_since_msg =
        nh_->now() - last_cmd_vel_msg_time_;
...
bool LocalPlanner::computeLocalPlan() {
  if (terrain_.isEmpty() || body_plan_msg_ == nullptr && !use_twist_input_ ||
      robot_state_msg_ == nullptr) {
    RCLCPP_WARN_STREAM(
        nh_->get_logger(),
  ...
  RCLCPP_INFO_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 100,
...
void LocalPlanner::spin() {
  ...
  while (rclcpp::ok()) {
    ...
    if (terrain_.isEmpty() ||
        (body_plan_msg_ == nullptr && !use_twist_input_) ||
        robot_state_msg_ == nullptr)
      continue;
