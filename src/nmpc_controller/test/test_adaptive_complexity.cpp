...
namespace {
struct RclcppInit {
  RclcppInit() { if (!rclcpp::ok()) rclcpp::init(0, nullptr); }
  ~RclcppInit() { if (rclcpp::ok()) rclcpp::shutdown(); }
};
static RclcppInit rclcpp_init;
}

TEST(NMPCTest, testAdaptiveComplexity) {
  // Load external params
  auto nh = rclcpp::Node::make_shared("nmpc_test_node");
  const int robot_id_ = 0;
  int N_;
  double dt_;
  nh->get_parameter_or("/local_planner/horizon_length", N_, 20);
  nh->get_parameter_or("/local_planner/timestep", dt_, 0.1);
