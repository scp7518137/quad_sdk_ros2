...
class quadNLP : public TNLP {
 public:
  ...
  /** Default constructor */
  quadNLP(rclcpp::Node::SharedPtr node, SystemID default_system, int N, double dt, double mu,
  ...
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_state_var(T &decision_var,
                                              const int &idx) const {
    return decision_var.segment(x_idxs_[idx], n_vec_[idx]);
  }

  // Get the idx-th body state variable from decision variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_body_state_var(T &decision_var,
                                                   const int &idx) const {
    return decision_var.segment(x_idxs_[idx], n_body_);
  }

  // Get the idx-th foot state variable from decision variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_foot_state_var(T &decision_var,
                                                   const int &idx) const {
    return decision_var.segment(x_idxs_[idx] + n_body_, n_foot_);
  }

  // Get the idx-th joint state variable from decision variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_joint_state_var(T &decision_var,
                                                    const int &idx) const {
    return decision_var.segment(x_idxs_[idx] + n_body_ + n_foot_, n_joints_);
  }

  // Get the idx-th control variable from decision variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_control_var(T &decision_var,
                                                const int &idx) const {
    return decision_var.segment(u_idxs_[idx], m_vec_[idx]);
  }

  // Get the idx-th body control variable from decision variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_body_control_var(T &decision_var,
                                                     const int &idx) const {
    return decision_var.segment(u_idxs_[idx], m_body_);
  }

  // Get the idx-th foot control variable from decision variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_foot_control_var(T &decision_var,
                                                     const int &idx) const {
    return decision_var.segment(u_idxs_[idx] + m_body_, m_foot_);
  }

  // Get the idx-th panic variable (for (idx+1)-th state variable) from decision
  // variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_slack_state_var(T &decision_var,
                                             const int &idx) const {
    return decision_var.segment(slack_state_var_idxs_[idx], 2 * n_slack_vec_[idx]);
  }

  // Get the idx-th panic variable (for (idx+1)-th constraint) from decision
  // variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_slack_constraint_var(T &decision_var,
                                                  const int &idx) const {
    return decision_var.segment(slack_constraint_var_idxs_[idx], 2 * g_slack_vec_[idx]);
  }

  // Get the idx-th constraint from constraint values
  template <typename T>
  inline Eigen::VectorBlock<T> get_primal_constraint_vals(T &constraint_vals,
                                                    const int &idx) const {
    return constraint_vals.segment(primal_constraint_idxs_[idx], g_vec_[idx]);
  }

  // Get the idx-th relaxed constraints from constraint values
  template <typename T>
  inline Eigen::VectorBlock<T> get_relaxed_primal_constraint_vals(
      T &constraint_vals, const int &idx) const {
    return constraint_vals.segment(relaxed_constraint_idxs_[idx], 2 * g_slack_vec_[idx]);
  }

  // Get the idx-th panic constraint (for (idx+1)-th state variable) from
  // constraint variable
  template <typename T>
  inline Eigen::VectorBlock<T> get_slack_constraint_vals(T &constraint_vals,
                                                   const int &idx) const {
    return constraint_vals.segment(slack_var_constraint_idxs_[idx], 2 * n_slack_vec_[idx]);
  }

  // Get the idx-th dynamic constraint related decision variable (idx and
  // idx+1-th state and idx-th control)
  template <typename T>
  inline Eigen::VectorBlock<T> get_dynamic_var(T &decision_var,
                                         const int &idx) const {
    return decision_var.segment(fe_idxs_[idx], n_vec_[idx] + m_vec_[idx] + n_vec_[idx + 1]);
  }

  // Get the idx-th dynamic constraint related jacobian nonzero entry
  template <typename T>
  inline Eigen::VectorBlock<T> get_dynamic_jac_var(T &jacobian_var,
                                             const int &idx) const {
    return jacobian_var.segment(dynamic_jac_var_idxs_[idx], nnz_mat_(sys_id_schedule_[idx], JAC));
  }

  // Get the idx-th dynamic constraint related jacobian nonzero entry
  template <typename T>
  inline Eigen::VectorBlock<T> get_relaxed_dynamic_jac_var(T &jacobian_var,
                                                     const int &idx) const {
    return jacobian_var.segment(
        relaxed_dynamic_jac_var_idxs_[idx], 2 * (relaxed_nnz_mat_(sys_id_schedule_[idx], JAC) + g_slack_vec_[idx]));
  }

  // Get the idx-th panic constraint jacobian (for (idx+1)-th state variable)
  // nonzero entry
  template <typename T>
  inline Eigen::VectorBlock<T> get_slack_jac_var(T &jacobian_var,
                                           const int &idx) const {
    return jacobian_var.segment(panic_jac_var_idxs_[idx], 4 * n_slack_vec_[idx]);
  }

  // Get the idx-th dynamic constraint related hessian nonzero entry
  template <typename T>
  inline Eigen::VectorBlock<T> get_dynamic_hess_var(T &hessian_var,
                                              const int &idx) const {
    return hessian_var.segment(dynamic_hess_var_idxs_[idx], nnz_mat_(sys_id_schedule_[idx], HESS));
  }

  // Get the idx-th state cost hessian nonzero entry
  template <typename T>
  inline Eigen::VectorBlock<T> get_state_cost_hess_var(T &hessian_var,
                                                 const int &idx) const {
    return hessian_var.segment(cost_idxs_[idx], n_cost_vec_[idx]);
  }

  // Get the idx-th control cost hessian nonzero entry
  template <typename T>
  inline Eigen::VectorBlock<T> get_control_cost_hess_var(T &hessian_var,
                                                   const int &idx) const {
    return hessian_var.segment(cost_idxs_[idx] + n_cost_vec_[idx], m_cost_vec_[idx]);
  }
