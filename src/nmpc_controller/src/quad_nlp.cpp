...
// Class constructor
quadNLP::quadNLP(rclcpp::Node::SharedPtr node, SystemID default_system, int N, double dt, double mu,
  ...
  quadKD_ = std::make_shared<quad_utils::QuadKD>(node);
...
void quadNLP::update_initial_guess(const quadNLP &nlp_prev, int shift_idx) {
  ...
  for (int i = 0; i < N_ - 1; i++) {
    ...
    if (n_vec_[i + 1] >
      ...
      RCLCPP_DEBUG(rclcpp::get_logger("nmpc_controller"),"Complexity at i + 1 = %d increased (i_prev + 1 = %d)", i + 1,
                i_prev + 1);
      ...
      if (n_vec_[i + 1] > nlp_prev.n_vec_[std::min(i + 1, nlp_prev.N_ - 1)]) {
        RCLCPP_DEBUG(rclcpp::get_logger("nmpc_controller"),
