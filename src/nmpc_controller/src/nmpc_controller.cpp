...
NMPCController::NMPCController(rclcpp::Node::SharedPtr nh, int robot_id) {
  nh_ = nh;
  robot_id_ = robot_id;
  ...
  nh_->get_parameter_or("/nmpc_controller/enable_mixed_complexity",
                        enable_mixed_complexity_, enable_mixed_complexity_);
  ...
  if (enable_mixed_complexity_) {
    ...
    nh_->get_parameter_or("/nmpc_controller/enable_adaptive_complexity",
                          enable_adaptive_complexity_, enable_adaptive_complexity_);
    // Define and load adaptive complexity parameters
    // (ROS2 integer array parameters use int64_t)
    std::vector<int64_t> fixed_complex_idxs;
    int fixed_complex_head = 0, fixed_complex_tail = 0;

    nh_->get_parameter_or("/nmpc_controller/fixed_complex_idxs",
                          fixed_complex_idxs, fixed_complex_idxs);
    nh_->get_parameter_or("/nmpc_controller/fixed_complex_head",
                          fixed_complex_head, fixed_complex_head);
    nh_->get_parameter_or("/nmpc_controller/fixed_complex_tail",
                          fixed_complex_tail, fixed_complex_tail);
    for (int64_t idx : fixed_complex_idxs) {
      if (idx >= 0 && idx <= N_) {
  ...
  mynlp_ = new quadNLP(nh_, default_system, N_, dt_, mu, panic_weights,
...
bool NMPCController::computePlan(
  ...
  if (status == Solve_Succeeded) {
    ...
    require_init_ = true;
    RCLCPP_WARN_STREAM(nh_->get_logger(), "NMPC solving fail");
    return false;
  }
}
